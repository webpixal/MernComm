        ...
        <li>
          ...
          rounded transition duration-75 hover:bg-[#57B846]" aria-controls="dropdown-example"
           data-collapse-toggle="dropdown-example">
            ...
            <span className="flex-1 text-left text-sm font-medium whitespace-nowrap" sidebar-toggle-item>{props.title}</span>
          ...
          <ul  className={`block space-y-2 ` + (isOpen? 'open' :'close')}>
            {props.data.map((listItem)=>{
                return (<li>
                    <Link to={listItem.labelLink} className="flex items-center p-1 pl-16 w-full text-base font-normal text-gray-200 rounded transition duration-75  hover:bg-[#57BB46] dark:text-white dark:hover:bg-gray-700">{listItem.label}</Link>
                  </li>)
            })}
          </ul>
        </li>
